...
export async function getOrderUser(email) {
    let urlJ = ('https://reactjs-532f1-default-rtdb.firebaseio.com/orders.json');
    let userUrl = (urlJ + `?orderBy="userEmail"&equalTo="${email}"&print=pretty`);
    let data = await fetch(userUrl);
    let response = await data.json();
    let all = [];
    if (response) {
        Object.keys(response).forEach(key => {
